...
void serialEvent()
{
  ...
  while (myPort.available()>0&&correct_serial_reads<8)//the x is there to make sure that a certain numer of lines commands are read from the serial port. 
...
void findMotors()//reading for the motor angles
{
  ...
  if (message1!=null&&message1.length==17) {
    for (int i=0; i<15; i+=2) {orientation_matrix[message1[i]-7][1] = byte(-1*message1[i+1]);}
